...
public class Kitchen {
	...
	public Kitchen(HashMap<String, Recipe> recipes, Recipe mainrecipe,
		...
		for(int i = 0; i<recipe.getMethods().size(); i++){
			if(recipe.getMethods().get(i).bakingdish != null && recipe.getMethods().get(i).bakingdish > maxdish)
				maxdish = recipe.getMethods().get(i).bakingdish;
			if (recipe.getMethods().get(i).mixingbowl != null && recipe.getMethods().get(i).mixingbowl > maxbowl)
				maxbowl = recipe.getMethods().get(i).mixingbowl;
		}
